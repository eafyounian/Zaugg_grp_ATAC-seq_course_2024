...
Let's repeat the same, but with a higher value for `minimal_branch_len`. If you want, create your own function that runs both `learn_graph` and `plot_cells` with `minimal_branch_len` as a paramter to avoid code redundancy!
