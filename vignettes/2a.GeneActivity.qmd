...
seu.s <- NormalizeData(
  ...
  scale.factor = median(seu.s@meta.data[,paste0("nCount_",newAssayName)]) #TODO: Replace without using @
)
...
markersGA <- FindAllMarkers(seu.s, only.pos = TRUE, min.pct = 0.1, logfc.threshold = 0.1, assay = newAssayName, verbose = F)
...
for (cluster.id in unique(top$cluster)){
  celltypes.l[[cluster.id]] = clustermole_overlaps(genes = topMarkers %>% filter(cluster == cluster.id) %>% pull(gene), species = "mm") %>% filter(species == "Mouse")
